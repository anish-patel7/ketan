...
import { Button, Select, Table, Tag, Modal, Input, message } from "antd";
import {
  Plus, Download, Eye,
  CheckCircle2, AlertTriangle, Snowflake, PackageSearch,
} from "lucide-react";
import {
  PROJECTS_LIST, MASTERSHEET, type DSRecord,
...
export default function DistributionPage() {
  ...
  // project gate — list only appears once a project is selected
  ...
  // ── Retrieval samples (subject rows from approved DS) ──

  const retrievalSamples = useMemo(() => {
    if (!retrievalDs) return [];
    // Use stored rows if available, else re-derive from mastersheet
  ...
  return (
    <AppLayout>
      <div className="page-container">
        ...
        {/* Project selector — gates the rest of the page */}
        ...
        {!selectedProject ? (
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <Button type="primary" icon={<Plus size={14} />} onClick={() => router.push(`/distribution/new?project=${selectedProject}`)}>
                New Distribution Sheet
              </Button>
            </div>
            ...
            <div style={{ padding:"12px 0" }}>
              <div className="rounded-xl p-4 mb-4" style={{ background:"var(--bg-card)", border:"1px solid var(--border)" }}>
                {[
                  ...
                  { label:"Run",       value:`#${approveDs.runNo}` },
